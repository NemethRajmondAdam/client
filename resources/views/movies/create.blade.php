<form action="{{ route('movies.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <fieldset class="mb-3">
        ...
        <small class="form-text text-muted">{{ __('Válasszon egy képet (opcionális)') }}</small>
    </fieldset>
    <!-- Length Field -->
    <fieldset class="mb-3">
        <label for="length" class="form-label">{{ __('Hossza (óra-perc, pl. 2-30)') }}</label>
        <input type="text" name="length" id="length" class="form-control" required placeholder="2-30" value="{{ old('length') }}">
    </fieldset>
    <!-- Release Date Field -->
    <fieldset class="mb-3">
        <label for="release_date" class="form-label">{{ __('Megjelenés éve') }}</label>
        <input type="number" name="release_date" id="release_date" class="form-control" min="1900" max="2100" required value="{{ old('release_date') }}">
    </fieldset>
